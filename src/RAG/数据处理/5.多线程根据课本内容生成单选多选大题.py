...
你是一个根据课本内容生成{type}的专家，给定一段{subject}{grade}的课本内容，请根据他们生成一道高考{type}。

# 步骤
- 利用你的知识，根据给定的课本内容，生成高考{type}，要求内容详细，覆盖全面
- 反复检查是否完整生成了题干、参考答案和解析，三者都要存在
- 反复检查题干、参考答案和解析是否和给定内容以及自身知识相一致，保持合理性和逻辑性和教学验证性

# 说明
- 出的题目绝对不能离开课本知识，要紧紧扣住课本内容
- 题目内容需包含题干、参考答案和解析
- 题干不能包含"课本"或者"课内"之类的词，不要显式说出来
- 题干需要完整，比如如果说结合材料，就需要把材料也列入题干，而非仅仅只有"结合材料"几个字
- 始终保持使用用户相同的语言
- 要求内容详细，覆盖全面
...
def get_response(example,prompt_template):
    ...
    intro = {
        ...
        "主观题":"主观题可以是单道小题或者是多道小题，每题要逐步增加难度，从识记到应用，并且在思考时每小题需要给出涉及的具体知识点。\n政治主观题和历史主观题出简答题；生物和地理主观题多为填空题，也可以有简答题，填空用___下划线表示需要填空的地方"
    }
    # exam = {
    ...
    try: 
        # [dict_[example['type']]]
        pr = prompt_template.replace("{inner}",example['课本内容'].strip()).replace("{grade}",str(example['grade']).strip()).replace("{subject}",example['subject'].strip()).replace("{type}",q_type)\
...
process(real_data,path = f"/hpc2hdd/home/fye374/ZWZ_Other/quizmanus/dataset/课本md/高中/md/合并/5.伪问答对（仅课本）（{q_type}）.jsonl")
