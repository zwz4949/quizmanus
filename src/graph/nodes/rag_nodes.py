...
def rag_generator(state: State):
    ...
    if generator_model == "qwen":
        SYSTEM_PROMPT = '''# 角色说明
你是一个根据课本内容和课外内容生成{type}的专家，给定一段{subject}的课本内容和一段相关的课外内容，请根据他们生成一道高考{type}。题目要完整，如果要引用材料信息就在题干包括具体的材料信息。不要在题目中包含'课本内容'、'课内知识'、'课外知识'等字眼.
...
解析：...'''
        ...
        SYSTEM_PROMPT = '''# 角色说明
你是一个根据课本内容和课外内容生成{type}的专家，给定一段{subject}的课本内容和一段相关的课外内容，请根据他们生成一道高考{type}。题目要完整，如果要引用材料信息就在题干包括具体的材料信息。不要在题目中包含课本内容、课内知识、课外知识等字眼，同时题目不能有根据某图片的信息，也不能包含图片。
